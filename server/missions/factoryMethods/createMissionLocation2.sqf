...
while {!_GotLoc} do 
{
	_randomIndex = random (count citySpawnMarkers - 1);

	//If the index of the mission markers array is false then break the loop and finish up doing the mission
	if (!((citySpawnMarkers select _randomIndex) select 1)) then 
	{
		_selectedMarker = citySpawnMarkers select _randomIndex select 0;
		_randomPos = getMarkerPos _selectedMarker;
        _returnData = [_randomPos,_randomIndex];
		citySpawnMarkers select _randomIndex set[1, true];
		_GotLoc = true;
	};
};
_returnData